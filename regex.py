...
# -*- coding: utf-8 -*-
"""
Common Regex Library Extracted from:
https://github.com/madisonmay/CommonRegex


:keyword  parser scraper data extraction metadata

"""
import re

mac_address = re.compile("[a-fA-F0-9]{2}[:][a-fA-F0-9]{2}[:][a-fA-F0-9]{2}[:][a-fA-F0-9]{2}[:][a-fA-F0-9]{2}[:][a-fA-F0-9]{2}")
...
etc_passwd = re.compile(r"^(.*):x:(\d+):(\d+):(.*?),?,?,?:(.*?):(.*)$", re.M)


# match everything between square brackets  [ ]: ex
# /"\[([^]]+)\]"/ "hello [world] [232] " -->  world, 232
#
